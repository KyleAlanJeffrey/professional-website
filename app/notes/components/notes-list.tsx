...
import { ChevronRight, File, Folder, PanelLeftClose, PanelLeft } from "lucide-react";
...
function FolderNode({
  node, depth = 0, activeSlug, onSelect,
}: {
  ...
  return (
    <div>
      ...
      >
        <ChevronRight className={`w-3 h-3 text-gray-400 dark:text-gray-500 transition-transform duration-100 shrink-0 ${open ? "rotate-90" : ""}`} />
      ...
      {open &&
        node.children.map((child, i) =>
          child.slug === null ? (
            <FolderNode key={child.name + i} node={child} depth={depth + 1} activeSlug={activeSlug} onSelect={onSelect} />
          ) : (
...
export default function NotesList({
  ...
  // Select first note on mount if none specified
  useEffect(() => {
    if (!activeSlug && notes.length > 0) {
  ...
  const handleSelect = (slug: string) => {
    setActiveSlug(slug);
    router.replace(`/notes?note=${encodeURIComponent(slug)}`, { scroll: false });
  };
  ...
  return (
    <div className="flex gap-4" style={{ height: "calc(100vh - 7rem)" }}>
      {/* Sidebar */}
      {sidebarOpen && (
        <div className="w-64 min-w-[16rem] shrink-0 rounded-xl border border-black/10 dark:border-white/[0.08] bg-white/70 dark:bg-white/[0.03] md:backdrop-blur overflow-y-auto">
          <div className="p-2">
            {tree.children.map((child, i) =>
              child.slug === null ? (
                <FolderNode key={child.name + i} node={child} activeSlug={activeSlug} onSelect={handleSelect} />
              ) : (
      ...
      {/* Content pane */}
      <div className="flex-1 min-w-0 flex flex-col rounded-xl border border-black/10 dark:border-white/[0.08] bg-white/70 dark:bg-white/[0.03] md:backdrop-blur overflow-hidden">
        {/* Toolbar */}
        <div className="flex items-center gap-3 px-4 py-2.5 border-b border-black/[0.06] dark:border-white/[0.06] shrink-0">
          <button
